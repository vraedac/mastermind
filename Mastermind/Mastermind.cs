...
namespace Mastermind
{
	...
	public class Mastermind
	{
		...
		private const string ErrString = "Input must be a string of four digits from 1 to 6, please try again.";
		...
		public (bool Result, string Hint) Guess(string input)
		{
			if (GuessesRemaining <= 0)
				throw new InvalidOperationException("This Mastermind game has been lost, instantiate a new one to continue playing.");
		...
		private int[] ConvertInputToGuessArray(string input)
		{
			...
			int[] checkDigits(int value)
			{
				// 7152
				if (value - 6000 >= 1000)
					throw new ArgumentOutOfRangeException(ErrString);

				// 2751
				var thx = value / 1000; // 2
				var hundreds = value - (1000 * thx); // 2751 - (1000 * 2) = 751
				if (hundreds - 600 >= 100) throw new ArgumentOutOfRangeException(ErrString); // 751 - 600 = 151

				// 2271
				var hx = hundreds / 100; // 2
				var tens = hundreds - (100 * hx); // 271 - (100 * 2) = 71
				if (tens - 60 >= 10) throw new ArgumentOutOfRangeException(ErrString);  // 71 - 60 = 11

				// 2457
				var tex = tens / 10; // 5
				var ones = tens - (10 * tex); // 57 - (10 * 5) = 7
				if (ones - 6 >= 1) throw new ArgumentOutOfRangeException(ErrString); // 7 - 6 = 1
